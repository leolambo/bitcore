...
import { CircuitBreaker } from '../../../providers/chain-state/external/circuitBreaker';
...
export class MultiProviderEVMStateProvider extends BaseEVMStateProvider {
   ...
   * Each network has its own sorted array of providers with independent circuit breakers.
  ...
  private initializeProviders(): void {
    ...
    // Iterate networks - each gets its own provider array with independent circuit breakers
      ...
      const providers = externalProviders
        .map((providerConfig) => ({
          ...
          circuitBreaker: new CircuitBreaker(
            ...
            { chain: this.chain, network } // Composite key: chain:network:provider
          ),
   ...
   * This catches the misconfiguration case where module is set to multiProvider
   * but externalProviders[] is empty or missing.
   */
  ...
  /**
   * Safe wrapper around adapter calls that guarantees circuit breaker permit release.
   * Prevents permit leaks for ANY throw (not just AdapterError), including unexpected
   * runtime errors like TypeError or RangeError inside adapter code.
   */
  private async withBreakerAttempt<T>(
    ...
    try {
      ...
      if (err instanceof InvalidRequestError) {
        // Bad input - don't trip breaker, don't failover. Release permit and re-throw.
        provider.circuitBreaker.releasePermit();
        throw err;
      }
      if (err instanceof AdapterError && err.isBreakerable) {
        provider.circuitBreaker.recordFailure(err);
      } else {
        // POLICY: Non-AdapterError (e.g., TypeError, RangeError inside adapter) is treated
        // as breaker-eligible. Rationale: if adapter code itself is broken, continuing to
        // route requests to it will keep failing. Recording failure ensures the circuit opens.
        provider.circuitBreaker.recordFailure(err);
      }
      return { error: err };
    }
  ...
  // Adapters return undefined for "not found" (indexing lag -> try next provider).
  // Adapters throw InvalidRequestError for bad input (stop immediately, no failover).
  // Adapters throw breaker-eligible errors for upstream failures (record failure, try next).
  async _getTransaction(params: StreamTransactionParams): Promise<{ tipHeight: number; found: any }> {
    ...
    let hadBreakerError = false;
    let attemptedAny = false;

    for (const provider of providers) {
      if (!provider.circuitBreaker.canAttempt()) {
        logger.debug(`MultiProvider: Skipping ${provider.adapter.name} for ${chain}:${network} (circuit ${provider.circuitBreaker.getState()})`);
        continue;
      }
      attemptedAny = true;

      // withBreakerAttempt guarantees permit release on any throw
      const attempt = await this.withBreakerAttempt(provider, () =>
        provider.adapter.getTransaction({ chain, network, chainId, txId })
      );

      if (attempt.error) {
        hadBreakerError = true;
  ...
  async _buildAddressTransactionsStream(params: StreamAddressUtxosParams) {
    ...
    // If startDate or endDate is provided, translate them to block numbers once (before
    // the provider loop) so all providers see consistent block-range args.
    // This uses the multi-provider _getBlockNumberByDate which itself does failover.
    ...
    // Stream permit tracking policy:
    // canAttempt() may grant a HALF_OPEN permit. For streams, the permit lifecycle is:
    //   - preflight success + stream end -> recordSuccess() (releases permit)
    //   - preflight success + stream error -> recordFailure() (releases permit)
    //   - preflight failure with AdapterError -> recordFailure() (releases permit)
    //   - preflight failure with non-AdapterError -> releasePermit() (releases without recording)
    //   - catch block for non-AdapterError -> releasePermit()
    // This ensures every canAttempt() permit is released exactly once.
    for (const provider of providers) {
      if (!provider.circuitBreaker.canAttempt()) continue;
      ...
      try {
        ...
        if (!preflight.success) {
          // Error before first data - can failover
          if (preflight.error instanceof AdapterError && preflight.error.isBreakerable) {
            provider.circuitBreaker.recordFailure(preflight.error);
          } else {
            // Non-AdapterError during preflight: still release permit
            provider.circuitBreaker.releasePermit();
          }
        ...
        // Permit lifecycle: exactly one of recordSuccess/recordFailure/releasePermit must fire.
        let permitSettled = false;
        const settlePermit = (type: 'success' | 'failure' | 'release', err?: Error) => {
          if (permitSettled) return;
          permitSettled = true;
          if (type === 'success') provider.circuitBreaker.recordSuccess();
          else if (type === 'failure' && err) provider.circuitBreaker.recordFailure(err);
          else provider.circuitBreaker.releasePermit();
        };

        txStream.on('error', (err) => {
          if (err instanceof AdapterError && err.isBreakerable) {
            settlePermit('failure', err);
          }
        });
        txStream.on('end', () => settlePermit('success'));
        // Client disconnect: release permit to prevent HALF_OPEN leak
        req!.on('close', () => settlePermit('release'));

        // Use PassThrough to prepend the buffered first item to the stream.
        // This avoids reliance on unshift() behavior with object-mode streams.
        let outputStream: Readable = txStream;
        if (preflight.firstItem) {
          const wrapper = new PassThrough({ objectMode: true });
          wrapper.write(preflight.firstItem); // Write buffered item first
          txStream.resume(); // Resume the paused source stream
          txStream.pipe(wrapper);
          outputStream = wrapper;
        } else {
          txStream.resume(); // Resume even if empty preflight (stream may have 'end')
        }

        // ExternalApiStream.onStream() signature: static onStream(stream: Readable, req, res, opts?)
        // It uses only standard Readable events (data, error, end), so PassThrough is valid.
        ...
        if (error instanceof AdapterError && (error as AdapterError).isBreakerable) {
          provider.circuitBreaker.recordFailure(error as Error);
        } else {
          provider.circuitBreaker.releasePermit();
        }
  ...
  private _preflightStream(stream: ExternalApiStream, timeoutMs: number): Promise<{
    ...
    return new Promise((resolve) => {
      ...
      const onData = (item: any) => {
        if (!resolved) {
          resolved = true;
          stream.pause(); // CRITICAL: pause immediately to prevent data loss
          cleanup();
      ...
      const onEnd = () => {
        if (!resolved) {
          resolved = true;
          cleanup();
          resolve({ success: true, firstItem: null }); // Empty result
        }
      };
  ...
  async _buildWalletTransactionsStream(params: StreamWalletTransactionsParams, streamParams: BuildWalletTxsStreamParams) {
    ...
    let activeProvider = providers.find(p => p.circuitBreaker.canAttempt());
    if (!activeProvider) {
    ...
    for (const address of walletAddresses) {
      try {
        ...
        activeProvider.circuitBreaker.recordFailure(error as Error);
        ...
        activeProvider = providers.find(p => p.circuitBreaker.canAttempt());
  ...
  // @override
  // Returns a candidate block from adapter, then enforces floor semantics via RPC
  // to guarantee: returned block's timestamp <= target date.
  async _getBlockNumberByDate(params: { date: Date; chainId: string | bigint; network?: string }) {
    const { date, chainId } = params;
    // Derive network from params or use first configured network as fallback
   ...
   * Caps adjustment iterations to prevent runaway RPC calls.
   */
  private async _enforceFloorSemantics(network: string, candidateBlock: number, date: Date): Promise<number> {
    ...
    // Check candidate block's timestamp
    let block = await web3.eth.getBlock(blockNum);
    if (!block) return blockNum;

    // If candidate is too late (timestamp > target), decrement
    let adjustments = 0;
    ...
    // Safety check: if we exhausted adjustments and still too late, the candidate was
    // far off. Fall back to RPC-only binary search to guarantee correctness.
  ...
  // They work correctly in external mode because they don't depend on indexed API providers.

  // Internal-only health check (NOT exposed as a public API endpoint).
  // Used for debugging/monitoring. May be exposed via admin-only route in future.
  async getProviderHealth(): Promise<Record<string, Record<string, any>>> {
    ...
    for (const [network, providers] of this.providersByNetwork.entries()) {
      health[network] = {};
      for (const provider of providers) {
        const metrics = provider.circuitBreaker.getMetrics();
