...
export class WalletStatsService {
  ...
  getSchedule(): { targetDay: number; targetHour: number } {
  ...
  async blockTimesByHeight(params: {
    chain: string;
    network: string;
    fromHeight: number;
    toHeight?: number;
  ...
  // Facts are written the same way whichever path collected them: keyed on the
  // unique {chain, network, snapshotDate, wallet} so a re-run is idempotent. The
  // SNAPSHOT halves deliberately differ — live overwrites, backfill inserts only.
  async persistWalletFacts(params: { chain: string; network: string; walletFacts: IWalletStatsWallet[] }) {
    const { chain, network, walletFacts } = params;
  ...
  async persist(params: { chain: string; network: string; snapshot: IWalletStats; walletFacts: IWalletStatsWallet[] }) {
    ...
    await this.persistWalletFacts({ chain, network, walletFacts });
